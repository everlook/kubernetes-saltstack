...
apt-get install -y build-essential dkms linux-headers-$(uname -r) lsb-release curl gnupg

# copy over mod/sysctl conf
cp /tmp/k8s.conf /etc/modules-load.d/k8s.conf
cp /tmp/containerd.conf /etc/modules-load.d/containerd.conf
cp /tmp/kubernetes.conf  /etc/sysctl.d/kubernetes.conf

# check mods/systcl loads
